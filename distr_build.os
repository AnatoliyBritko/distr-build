...
Процедура ЗадатьНачальныеНастройки()
	...
	Для НомерСтроки = 1 По ТекстовыйДокумент.КоличествоСтрок() Цикл
		...
		ИначеЕсли НомерСтроки = 9 тогда
			СтруктураНастроек.Вставить("КаталогРезультатовТестов",Стр);
		ИначеЕсли НомерСтроки = 10 тогда
		СтруктураНастроек.Вставить("ПрефиксСоздаваемогоРелиза",Стр);
...
Процедура СоздатьФайлПоставки()
	...
	Если МассивДистрибутивов.Количество() > 0 Тогда
		...
		СтруктураНастроек.Вставить("НомерСоздаваемогоРелизаCFU",НомерСоздаваемогоРелизаCFU);
...
// Процедура выполняет проверку значений в котроль файлах - результатах выполнения операций по созданию релиза.
// При  наличии ошибок в файлах будет создан файл, с наименованием "Error", а в самом файле перечислены эапы на которых возникли Ошибки 
// Если ошибок не будет обнаружено, то будет создан файл с наименованием "All right, boss" и значением "0"
Процедура ВыполнитьИтоговуюПроверкуНаличияОшибок()
	
	МассивОшибок = Новый Массив;
	
	МассивФайловПроверок = Новый Массив;
	МассивФайловПроверок.Добавить("CheckConfig");
	МассивФайловПроверок.Добавить("CheckModules");
	МассивФайловПроверок.Добавить("ConfigurationRepositoryBindCfg");
	МассивФайловПроверок.Добавить("ConfigurationRepositoryCommit");
	МассивФайловПроверок.Добавить("ConfigurationRepositoryLock");
	МассивФайловПроверок.Добавить("CreateDistributionFiles");
	МассивФайловПроверок.Добавить("MergeCfg");
	
	КаталогФайла = КаталогЛогов; 
	//СтруктураНастроек.КаталогСборки;//"c:\Les82\acf5cbc8-37f2-44f5-9516-b29b908ae1aa\-=sandbox=-\DumpResults\";
	
	ЕстьОшибкиСозданияРелиза = Ложь;
	
	Для Каждого МассивИмяФала Из МассивФайловПроверок Цикл
		
		ИмяФайла = КаталогФайла + МассивИмяФала + ".txt";
		ТекстовыйДокумент = Новый ТекстовыйДокумент;
		Попытка
			ТекстовыйДокумент.Прочитать(ИмяФайла);
		Исключение
			//Сообщить("Ошибка открытия файла!");
			МассивОшибок.Добавить(МассивИмяФала);
			ЕстьОшибкиСозданияРелиза = Истина;
			Продолжить;
		КонецПопытки;
		
		КолСтрокДокумента = ТекстовыйДокумент.КоличествоСтрок();
		Если КолСтрокДокумента >  1 Тогда 
			ЕстьОшибкиСозданияРелиза = Истина;
			МассивОшибок.Добавить(МассивИмяФала);
		КонецЕсли;
		
		Для НомерСтроки = 1 По КолСтрокДокумента Цикл
			Стр	= ТекстовыйДокумент.ПолучитьСтроку(НомерСтроки);   
			Стр = СокрЛП(Стр);
			Если Стр <> "0" Тогда
				ЕстьОшибкиСозданияРелиза = Истина;
	...
	Если ЕстьОшибкиСозданияРелиза Тогда
		ИмяФайлаОшибок = "Error";
	Иначе
		ИмяФайлаОшибок = "All right, boss";
		МассивОшибок.Добавить("0");
	КонецЕсли;	
	
	Для Каждого ОшибкаМассива Из МассивОшибок Цикл
		Текст.ДобавитьСтроку(ОшибкаМассива);
	КонецЦикла;
	
	Текст.Записать(КаталогФайла + ИмяФайлаОшибок+ ".txt");

	// Записываем также файл в каталог релизов.
	Текст.Записать(СтруктураНастроек.КаталогСоздаваемогоРелиза + ИмяФайлаОшибок+ ".txt");

	КопироватьСодержимоеКаталога(КаталогЛогов,СтруктураНастроек.КаталогРезультатовТестов)
...
Процедура ВыполнитьКопированиеФайлов()
	//1. Создаем папку с релизов в каталоге релизов.

	мДатаРелиза = ТекущаяДата();
	ДатаГод 	= СтрЗаменить(Год(мДатаРелиза)," ","");
	ДатаМесяц 	= Строка(Месяц(мДатаРелиза));
	Если СтрДлина(ДатаМесяц) = 1 Тогда
		ДатаМесяц 	= "0" + ДатаМесяц;
	КонецЕсли;

	ДатаДень = День(мДатаРелиза);
	Если СтрДлина(ДатаДень) = 1 Тогда
		ДатаДень 	= "0" + ДатаДень;
	КонецЕсли;

	мДатаРелизаПропись = ДатаГод + "-" + ДатаМесяц + "-" + ДатаДень;

	ИмяПапки = "Обновление " + мДатаРелизаПропись + " " + СтруктураНастроек.ПрефиксСоздаваемогоРелиза + " релиз " + СтруктураНастроек.НомерСоздаваемогоРелиза;
	ИмяКаталога = СтруктураНастроек.КаталогРелизов + "\" + ИмяПапки; 
	СоздатьКаталог(ИмяКаталога);

	СтруктураНастроек.Вставить("КаталогСоздаваемогоРелиза",ИмяКаталога+"\");

	//2. Копируем файлы обновлений.
	КаталогИсточник = СтруктураНастроек.КаталогСборки;
	КаталогПриемник = СтруктураНастроек.КаталогСоздаваемогоРелиза;

	ИмяФайла  = СтруктураНастроек.НомерСоздаваемогоРелиза + ".cf";

	ПереместитьФайл(КаталогИсточник + ИмяФайла,КаталогПриемник + ИмяФайла);
	
	

	ИмяФайла  = СтруктураНастроек.НомерСоздаваемогоРелизаCFU + ".cfu";
	
	ПереместитьФайл(КаталогИсточник + ИмяФайла,КаталогПриемник + ИмяФайла);
	
	//3. Копируем итоговый файл по выполненым проверкам.
	//реализовано в процедуре ВыполнитьИтоговуюПроверкуНаличияОшибок();
		
	//4.  Создаем текстовый файл с напомимнанием сделать бэкап.
	Текст = Новый ТекстовыйДокумент;
	ИмяФайла = "Перед установкой обновления обязательно делайте архив базы данных !!!";
	Текст.ДобавитьСтроку(ИмяФайла);
	Текст.Записать(СтруктураНастроек.КаталогСоздаваемогоРелиза + ИмяФайла+ ".txt");


	//5. Копируем файлы со списком тестов
	ИмяКаталога = СтруктураНастроек.КаталогРезультатовТестов + "\" + ИмяПапки;
	СоздатьКаталог(ИмяКаталога);
	СтруктураНастроек.КаталогРезультатовТестов = ИмяКаталога + "\";
...
Процедура ВыполнитьРаботыНаХранилище()
	...
	ВыполнитьИтоговуюПроверкуНаличияОшибок();

	// Удаляем каталоги, в которых производилась сборка релизов
	УдалитьФайлы(СтруктураНастроек.КаталогСборки);
